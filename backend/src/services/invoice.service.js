...
export const updateInvoice_s = async (invoice, data) => {
    ...
    if(totalpaid.totale >= invoice.amount) {
        throw new AppError('Cannot modify an invoice that is already fully paid', 422)
    }
    ...
    invoice.status = invoiceStatus(invoice.amount, invoice.dueDate, totalpaid.totale);

    const invoiceUpdated = await Invoices.findByIdAndUpdate({_id: invoice.id}, invoice, { returnDocument: 'after' });
...
export const deleteInvoice_s = async (invoiceId, userId) => {
    ...
    if(paymentCount > 0) {
        return new AppError(`cannot delete invoice it has ${paymentCount} payment recorded`, 422)
    }
